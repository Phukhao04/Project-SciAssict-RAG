...
from app.schemas.auth import AuthenRequestBody, AccessRequestBody, AuthResponse, RegisterRequest, RegisterResponse
from app.crud.auth_crud import check_authen_request, check_access_request, create_user, username_exists
...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="ไม่สามารถสมัครสมาชิกได้ กรุณาตรวจสอบข้อมูล") from exc
...
def authen_request(body: AuthenRequestBody, db: Session = Depends(get_db)):
    ...
    payload = {"user_id": result["data"]["user_id"], "username": result["data"]["username"]}
